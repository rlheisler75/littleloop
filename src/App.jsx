...
function getSitterParam() {
  const p = new URLSearchParams(window.location.search);
  return p.get("sitter") || null;
}

function getBrowseParam() {
  const p = new URLSearchParams(window.location.search);
  return p.get("browse") !== null;
}
...
function SitterFamilyDetail({family,children,sitterId,sitterName,onDeactivate}) {
  ...
  useEffect(()=>{
    ...
    const ch=supabase.channel(`eta-${family.id}`)
      ...
      .subscribe();
    // Auto-clear expired ETAs every 30 seconds
    const timer=setInterval(()=>{
      setEtas(prev=>prev.filter(e=>new Date(e.expires_at)>new Date()));
    },30000);
...
function PublicProfileEditor({sitterId, sitterName, sitterCity, sitterState}) {
  ...
  function copyLink() {
    navigator.clipboard.writeText(`https://www.littleloop.xyz/?sitter=${username}`);
    setCopied(true);
  ...
  return (
    <div className="card" style={{padding:"20px 18px",marginBottom:14}}>
      ...
      <div style={{fontSize:12,color:'var(--text-faint)',marginBottom:16}}>
        Let families discover you on littleloop. Your profile will be searchable by city/state.
      </div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,padding:'12px 14px',background:'var(--card-bg)',borderRadius:10,border:'1px solid var(--border)'}}>
        ...
        <div onClick={()=>setPub(v=>!v)} style={{
          ...
          background: pub ? 'var(--accent)' : 'rgba(255,255,255,.1)',
          position:'relative'
        }}>
          <div style={{
        ...
        <form onSubmit={save}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              <input className="fi" value={username} onChange={e=>setUsername(e.target.value.toLowerCase())}
                placeholder="your-name" style={{flex:1}}/>
            </div>
            {username && (
              <button type="button" onClick={copyLink} className="bg"
          ...
          <div style={{marginBottom:12}}>
            ...
            <textarea className="fi" value={bio} onChange={e=>setBio(e.target.value)}
              placeholder="Tell families a little about yourself, your experience, and what makes you a great sitter..."
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              <input className="fi" type="number" value={rateMin} onChange={e=>setRateMin(e.target.value)}
                placeholder="Min" style={{width:80}}/>
              <div style={{fontSize:12,color:'var(--text-faint)'}}>–</div>
              <input className="fi" type="number" value={rateMax} onChange={e=>setRateMax(e.target.value)}
                placeholder="Max" style={{width:80}}/>
          ...
          <div style={{marginBottom:12}}>
            ...
            <input className="fi" type="number" value={years} onChange={e=>setYears(e.target.value)}
              placeholder="e.g. 3" style={{width:120}}/>
          </div>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {AGE_RANGES.map(r=>(
                <div key={r} onClick={()=>toggleAgeRange(r)} style={{
                  padding:'4px 10px',borderRadius:20,fontSize:11,cursor:'pointer',
                  background: ageRanges.includes(r) ? 'var(--accent)' : 'var(--card-bg)',
                  border: `1px solid ${ageRanges.includes(r) ? 'var(--accent)' : 'var(--border)'}`,
                  color: ageRanges.includes(r) ? '#fff' : 'var(--text-dim)',
                }}>{r}</div>
              ))}
            </div>
          </div>
          ...
          <div style={{marginBottom:16}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {CERTS.map(c=>(
                <div key={c} onClick={()=>toggleCert(c)} style={{
                  padding:'4px 10px',borderRadius:20,fontSize:11,cursor:'pointer',
                  background: certs.includes(c) ? 'rgba(60,180,100,.2)' : 'var(--card-bg)',
                  border: `1px solid ${certs.includes(c) ? 'rgba(60,180,100,.5)' : 'var(--border)'}`,
                  color: certs.includes(c) ? '#5EE89A' : 'var(--text-dim)',
                }}>{c}</div>
              ))}
            </div>
          </div>}
...
function PublicSitterProfile({username}) {
  const [sitter, setSitter]   = useState(null);
    ...
    <div style={{maxWidth:640,margin:'0 auto',padding:'24px 16px',position:'relative',zIndex:1}}>
      ...
      <div className="card" style={{padding:'24px 20px',marginBottom:16}}>
        <div style={{display:'flex',gap:16,alignItems:'flex-start',marginBottom:16}}>
          <div style={{width:72,height:72,borderRadius:'50%',background:'var(--accent)',
            ...
            {sitter.avatar_url ? <img src={sitter.avatar_url} style={{width:'100%',height:'100%',borderRadius:'50%',objectFit:'cover'}}/> : '➿'}
          </div>
          <div style={{flex:1}}>
            ...
            {avgRating && (
              <div style={{display:'flex',alignItems:'center',gap:6,marginTop:4}}>
                ...
                <span style={{fontSize:12,color:'var(--text-dim)'}}>{avgRating} ({reviews.length} review{reviews.length!==1?'s':''})</span>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Stats row */}
        <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:sitter.bio?16:0}}>
          {sitter.years_experience && (
            <div style={{padding:'6px 12px',borderRadius:20,background:'var(--card-bg)',border:'1px solid var(--border)',fontSize:12}}>
              🏅 {sitter.years_experience} yr{sitter.years_experience!==1?'s':''} experience
            </div>
          )}
          {(sitter.hourly_rate_min||sitter.hourly_rate_max) && (
            <div style={{padding:'6px 12px',borderRadius:20,background:'var(--card-bg)',border:'1px solid var(--border)',fontSize:12}}>
              💰 ${sitter.hourly_rate_min||'?'}–${sitter.hourly_rate_max||'?'}/hr
            </div>
          )}
          {sitter.availability && (
            <div style={{padding:'6px 12px',borderRadius:20,background:'var(--card-bg)',border:'1px solid var(--border)',fontSize:12}}>
      ...
      {sitter.age_ranges?.length > 0 && (
        <div className="card" style={{padding:'16px 20px',marginBottom:16}}>
          <div style={{fontSize:13,fontWeight:600,marginBottom:10}}>👶 Works With</div>
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            {sitter.age_ranges.map(r=>(
              <div key={r} style={{padding:'4px 12px',borderRadius:20,fontSize:12,
                background:'rgba(58,111,212,.15)',border:'1px solid rgba(58,111,212,.3)',color:'var(--accent)'}}>{r}</div>
            ))}
          </div>
        </div>
      )}
      ...
      {sitter.certifications?.length > 0 && (
        <div className="card" style={{padding:'16px 20px',marginBottom:16}}>
          <div style={{fontSize:13,fontWeight:600,marginBottom:10}}>✅ Certifications & Skills</div>
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            {sitter.certifications.map(c=>(
              <div key={c} style={{padding:'4px 12px',borderRadius:20,fontSize:12,
                background:'rgba(60,180,100,.15)',border:'1px solid rgba(60,180,100,.3)',color:'#5EE89A'}}>{c}</div>
            ))}
          </div>
        </div>
      )}
      ...
      {/* Reviews */}
      <div className="card" style={{padding:'16px 20px',marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:600,marginBottom:reviews.length?12:0}}>
          ⭐ Reviews {reviews.length > 0 && <span style={{fontWeight:400,color:'var(--text-faint)'}}>({reviews.length})</span>}
        </div>
        {reviews.length === 0 ? (
          <div style={{fontSize:12,color:'var(--text-faint)',marginTop:8}}>No reviews yet.</div>
        ) : reviews.map(r=>(
      ...
      <div className="card" style={{padding:'20px',textAlign:'center'}}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,marginBottom:8}}>
          Ready to connect with {sitter.name.split(' ')[0]}?
        </div>
        <div style={{fontSize:12,color:'var(--text-faint)',marginBottom:14}}>
          Join littleloop to send a message and manage your childcare all in one place.
        </div>
        <button className="bp" onClick={()=>window.location.href='/?portal=parent'}>
    ...
    <div style={{maxWidth:720,margin:'0 auto',padding:'24px 16px',position:'relative',zIndex:1}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:24}}>
        <div style={{flex:1}}>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600}}>
            <span className="leaf">➿</span> Find a Sitter
          </div>
          <div style={{fontSize:13,color:'var(--text-faint)'}}>Browse available sitters in your area</div>
        </div>
        <button className="bp" onClick={()=>window.location.href='/?portal=parent'} style={{fontSize:12,whiteSpace:'nowrap'}}>
          Sign Up Free
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{padding:32,textAlign:'center'}}>
          ...
          <div style={{fontSize:12,color:'var(--text-faint)'}}>
            {search ? 'Try a different search.' : 'No sitters have created public profiles yet.'}
          </div>
        </div>
      ) : (
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          {filtered.map(s=>{
            return (
              <div key={s.id} className="card fc" style={{padding:'16px 18px',cursor:'pointer'}}
                onClick={()=>window.location.href=`/?sitter=${s.username}`}>
                <div style={{display:'flex',gap:14,alignItems:'flex-start'}}>
                  <div style={{width:52,height:52,borderRadius:'50%',background:'var(--accent)',
                    display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>
                    {s.avatar_url ? <img src={s.avatar_url} style={{width:'100%',height:'100%',borderRadius:'50%',objectFit:'cover'}}/> : '➿'}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {s.bio && (
                      <div style={{fontSize:12,color:'var(--text-dim)',marginTop:4,
                        overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s.bio}</div>
                    )}
                    <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:6}}>
                      {s.years_experience && (
                        <span style={{fontSize:11,padding:'2px 8px',borderRadius:10,
                          background:'var(--card-bg)',border:'1px solid var(--border)'}}>
                          🏅 {s.years_experience}yr exp
                        </span>
                      )}
                      {(s.hourly_rate_min||s.hourly_rate_max) && (
                        <span style={{fontSize:11,padding:'2px 8px',borderRadius:10,
                      ...
                      {s.age_ranges?.slice(0,2).map(r=>(
                        <span key={r} style={{fontSize:11,padding:'2px 8px',borderRadius:10,
                          background:'rgba(58,111,212,.1)',border:'1px solid rgba(58,111,212,.2)',color:'var(--accent)'}}>
                          {r}
                        </span>
                      ))}
                    </div>
      ...
      <div style={{textAlign:'center',marginTop:32,padding:'20px',borderTop:'1px solid var(--border)'}}>
        <div style={{fontSize:13,color:'var(--text-faint)',marginBottom:10}}>
          Are you a sitter? Create your free public profile.
        </div>
...
// ─── Leave a Review (family side) ────────────────────────────────────────────
function LeaveReviewModal({open, onClose, sitterId, sitterName, familyId, reviewerId, reviewerName}) {
  ...
  async function save() {
    ...
    const payload = {sitter_id:sitterId,family_id:familyId,reviewer_id:reviewerId,
      reviewer_name:reviewerName,rating,review_text:text.trim()||null};
    const {error} = existing
              ...
              :<>
                ...
                <div className="card fade-up" style={{padding:24,marginBottom:16}}>
                        ...
                        {(family.sitters_list||[]).map(s=>(
                          <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",
                            ...
                            {s.connection_status==="active"&&(
                              <button className="bg" style={{fontSize:10,padding:'3px 8px'}}
                                onClick={()=>setReviewTarget({sitterId:s.id,sitterName:s.name})}>
                                ⭐ Review
                              </button>
                            )}
                            {s.connection_status!=="active"&&(
                  ...
                  {reviewTarget&&<LeaveReviewModal
                    ...
                    familyId={family.id} reviewerId={session.user.id}
                    reviewerName={member?.name||name}
                  />}

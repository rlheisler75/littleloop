...
// Global CSS injected once in App.jsx via:
//   const tag = document.createElement('style');
//   tag.textContent = CSS;
//   document.head.appendChild(tag);
//
// All color values use CSS custom properties set by applyTheme() in src/lib/theme.js.
// NEVER use hardcoded rgba(255,255,255,...) or rgba(20,36,58,...) in components —
// always use var(--text), var(--text-dim), var(--text-faint) so light themes work.
...
export const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'DM Sans',sans-serif;background:var(--body-bg,#0C1420);color:var(--text,#E4EAF4);min-height:100vh;overflow-x:hidden;transition:background .3s,color .3s}
  ...
  @keyframes themeSwitch{from{opacity:.7}to{opacity:1}}
  ...
  .theme-switching{animation:themeSwitch .25s ease both}

  /* ── Logo ── */
  .logo-text{
    font-family:'Cormorant Garamond',serif;
    font-size:42px;font-weight:700;letter-spacing:-1px;
    background:var(--logo-grad,linear-gradient(90deg,#6FA3E8,#A8CCFF,#E2EDFF,#A8CCFF,#6FA3E8));
  ...
  .card{
    ...
    box-shadow:var(--shadow,0 20px 60px rgba(0,0,0,.2)),inset 0 1px 0 rgba(255,255,255,.06)
  }
  ...
  .nav-tab{
    ...
    color:var(--nav-text,rgba(255,255,255,.3));
    border-bottom:2px solid transparent;transition:all .2s;
    display:flex;flex-direction:column;align-items:center;gap:3px
  }
  .nav-tab:hover{color:var(--text-dim,rgba(255,255,255,.6))}
  .nav-tab.active{color:var(--accent,#7BAAEE);border-bottom-color:var(--accent,#7BAAEE)}
  ...
  .tab:hover{color:var(--text-dim,rgba(255,255,255,.5))}
  .tab.active{color:var(--accent,#7BAAEE);border-bottom-color:var(--accent,#7BAAEE)}

  /* ── Form elements ── */
  .fl{
    display:block;font-size:10px;font-weight:600;letter-spacing:.9px;text-transform:uppercase;
    color:var(--text-faint,rgba(255,255,255,.3));margin-bottom:7px
  }
  .fi{
    width:100%;padding:11px 14px;
    background:var(--input-bg,rgba(255,255,255,.04));
    border:1px solid var(--input-border,rgba(255,255,255,.09));
    border-radius:10px;color:var(--text,#E4EAF4);
    font-size:13px;outline:none;transition:all .2s;margin-bottom:14px
  }
  .fi::placeholder{color:var(--text-faint,rgba(255,255,255,.2))}
  .fi:focus{border-color:rgba(111,163,232,.45);background:rgba(111,163,232,.06);box-shadow:0 0 0 3px rgba(111,163,232,.1)}
  select.fi option{background:var(--body-bg,#0C1420);color:var(--text,#E4EAF4)}

  /* ── Buttons ── */
  .bp{
    padding:11px 20px;
    background:var(--accent-grad,linear-gradient(135deg,#3A6FD4,#2550A8));
    border:none;border-radius:10px;
    color:var(--btn-text,#fff);
    font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;
    display:inline-flex;align-items:center;gap:7px
  }
  .bp:hover:not(:disabled){filter:brightness(1.1);transform:translateY(-1px);box-shadow:0 6px 20px rgba(0,0,0,.25)}
  ...
  .bg{
    ...
    border-radius:10px;color:var(--text-dim,rgba(255,255,255,.5));
  ...
  .bd{
    padding:8px 14px;
    background:rgba(192,80,80,.12);border:1px solid rgba(192,80,80,.25);
    border-radius:9px;color:#F5AAAA;font-size:12px;cursor:pointer;transition:all .2s;
    display:inline-flex;align-items:center;gap:6px
  }
  ...
  .fc:hover{background:var(--card-bg);border-color:rgba(111,163,232,.25)}
  .fc.active{background:rgba(111,163,232,.08);border-color:rgba(111,163,232,.35)}
  ...
  .chip{display:inline-flex;align-items:center;gap:6px;background:var(--pill-bg,rgba(255,255,255,.05));border:1px solid var(--border,rgba(255,255,255,.08));border-radius:20px;padding:5px 11px;font-size:12px;color:var(--pill-text,#E4EAF4)}

  /* ── Scrollbar ── */
  ::-webkit-scrollbar{width:3px}
  ::-webkit-scrollbar-thumb{background:var(--border,rgba(255,255,255,.1));border-radius:4px}
  ...
  .theme-swatch:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(0,0,0,.25)}
  .theme-swatch:focus-visible{box-shadow:0 0 0 3px var(--accent)}
  .theme-swatch-bar{
  ...
  @media(max-width:600px){
    ...
    .theme-picker-grid{grid-template-columns:repeat(3,1fr);gap:6px}
  }
`;

    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-family`,
        {
          ...
          body: JSON.stringify({ familyName: familyName.trim(), userName }),
        }
      );
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        {step === 0 && (
          <div className="card" style={{ padding: '32px 28px', textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 600, marginBottom: 10 }}>Welcome, {userName}!</div>
            <p style={{ fontSize: 13, color: 'var(--text-faint)', lineHeight: 1.7, marginBottom: 28 }}>
              littleloop connects your family with your childcare provider. Let's get your family set up.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 28, textAlign: 'left' }}>
              {[
                ...
                <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 10, background: 'var(--input-bg)', border: '1px solid var(--border)' }}>
                  <span style={{ fontSize: 18 }} dangerouslySetInnerHTML={{ __html: f.icon }}/>
                  <span style={{ fontSize: 12, color: 'var(--text-faint)' }}>{f.label}</span>
                </div>
              ))}
            </div>
            <button className="bp full" onClick={() => setStep(1)}>Get started &rarr;</button>
          </div>
        )}
        {step === 1 && (
          <div className="card" style={{ padding: '32px 28px' }}>
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 600, marginBottom: 4 }}>Name your family</div>
            <p style={{ fontSize: 13, color: 'var(--text-faint)', marginBottom: 24, lineHeight: 1.6 }}>
              This is how your sitter will see you &mdash; usually your last name works great.
            </p>
            <div style={{ marginBottom: 24 }}>
              ...
              <input
                ...
                autoFocus
                onKeyDown={e => e.key === 'Enter' && !saving && familyName.trim() && createFamily()}
              />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
              <button className="bg" onClick={() => setStep(0)}>&larr; Back</button>
              <button className="bp" onClick={createFamily} disabled={saving || !familyName.trim()}>
                {saving ? <Spinner/> : 'Create My Family \u2192'}
              </button>
            </div>
          </div>
        )}
...
export default function ParentDashboard({ session, onSignOut }) {
  ...
  useEffect(() => {
    const userId = session.user.id;
    if (tab === 'messages') { localStorage.setItem(`ll_seen_msg_${userId}`, new Date().toISOString()); setUnread(u => ({ ...u, messages: 0 })); }
    if (tab === 'invoices' && family) { localStorage.setItem(`ll_seen_inv_${family.id}_${userId}`, new Date().toISOString()); setUnread(u => ({ ...u, invoices: 0 })); }
    if (tab === 'feed' && family) { localStorage.setItem(`ll_seen_feed_${family.id}_${userId}`, new Date().toISOString()); setUnread(u => ({ ...u, feed: 0 })); }
  }, [tab, family]);
  ...
  if (!member) return <><Bg/><NewFamilySetup session={session} onComplete={load} onSignOut={onSignOut}/></>;
        ...
        {NAV.map(n => (
          <div key={n.id} className={`nav-tab ${tab === n.id ? 'active' : ''}`} onClick={() => setTab(n.id)}>
            <span style={{ position: 'relative', display: 'inline-block' }}>
              <span style={{ fontSize: 18 }} dangerouslySetInnerHTML={{ __html: n.icon }}/>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', maxWidth: 800, width: '100%', margin: '0 auto' }}>
        {tab === 'home' && (
          <div>
            {family ? (
              <>
                {family.id && <div style={{ marginBottom: 20 }}><ParentFollowUs familyId={family.id}/></div>}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  ...
                  <div style={{ flex: 1 }}><FamilyNameEditor familyId={family.id} name={family.name} onSaved={n => setFamily(f => ({ ...f, name: n }))}/></div>
                </div>
                ...
                {(isAdmin || member?.role === 'member') && <OnMyWayButton familyId={family.id} memberId={member?.id} memberName={name}/>}
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {family.sitters_list.map(s => (
                          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--input-bg)', borderRadius: 12, border: '1px solid var(--border)' }}>
                            ...
                            <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500 }}>{s.name}</div></div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {members.map(m => (
                      <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--card-bg)', borderRadius: 12, border: '1px solid rgba(255,255,255,.06)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          {m.photo_url ? <img src={m.photo_url} style={{ width: 28, height: 28, borderRadius: 8, objectFit: 'cover' }} alt={m.name}/> : <span style={{ fontSize: 22 }}>{m.avatar || '&#x1F464;'}</span>}
                          <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                          ...
                          {(isAdmin || m.user_id === session.user.id) && <button className="bg" style={{ padding: '4px 8px', fontSize: 11 }} onClick={() => setEditMember(m)}>&#x270F;&#xFE0F;</button>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="es"><div className="ic">&#x1F3E0;</div><h3>No family found</h3><p>Your account is not linked to a family yet.</p></div>
            )}
          </div>
        )}

    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 0 40px', position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ marginBottom: 20, padding: '16px', borderRadius: 14, background: 'var(--card-bg)', border: '1px solid var(--border)', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 12 }}>Join littleloop to send a message and manage your childcare in one place.</div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', position: 'relative', zIndex: 1 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 28, fontWeight: 600 }}>Find a Sitter ➿</div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
        <input className="fi" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, city, or keyword…" style={{ flex: 1, minWidth: 200, marginBottom: 0, fontSize: 13 }}/>
        ...
        : filtered.length === 0
          ...
          : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {filtered.map(s => (
                <div key={s.id} className="card fc" style={{ padding: 0, overflow: 'hidden', cursor: 'pointer' }} onClick={() => window.location.href = `/?sitter=${s.username}`}>
                  <div style={{ display: 'flex', gap: 0 }}>
                    {/* Avatar */}
                    <div style={{ width: 80, flexShrink: 0, background: 'var(--card-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>
                      {s.avatar_url
                        ? <img src={s.avatar_url} style={{ width: 80, height: '100%', minHeight: 100, objectFit: 'cover' }}/>
                        : '➿'
                      }
                    </div>
                    {/* Info */}
                    <div style={{ flex: 1, padding: '14px 16px', minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
                        ...
                        {(s.services || []).slice(0, 2).map(svc => <Tag key={svc} label={svc}/>)}
                      </div>
                    </div>
                  </div>
                </div>
